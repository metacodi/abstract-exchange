...
export abstract class AbstractExchangeCopy extends TaskExecutor {
  /** Nombre del exchange */
  name: ExchangeType;
  ...
  protected processExchangeLimits(rateLimits: Limit[]): void {
    // NOTA: Si no hi ha límits per processar, sortim, sinó obtindrem un error durant la crida a `findLimit` per falta d'inicialització al reduce().
